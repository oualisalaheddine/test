...
public class OperationService {
	...
	/**
	 * Retourne une instance d'Operation pré-remplie à partir d'une opération
	 * interne existante. Si l'ID est null, on retourne simplement un nouvel objet
	 * vide.
	 */
	/// private Integer interId;
	public Operation initFromInterneI(Integer interId) {
		/**
		 * if (interId == null) {
		 * 
		 ...
		 * operationEnCoursInterneService.findById(interId) .orElse(((null)));
		 * 
		 * System.out.println("object interne -------------" + inter);
		 * ///System.out.println("object intern -------------" +
		 * inter.getClass().getSimpleName()); OperationEnCoursExterne exter =
		 * operationEncoursExterneService.findById(interId) .orElse(((null)));
		 * 
		 * System.out.println("object ixter -------------" + exter); Operation op = new
		 * Operation();
		 * 
		 * if (inter!= null ){
		 ...
		 * 
		 * System.out.println("object interne -------------" + inter); }else {
		 * op.setImmatriculation(exter.getVehiculeExterne().getImmatriculation());
		 * op.setPeser2(exter.getPeser1()); op.setOperationEnCoursId(interId);
		...
		// Optional<OperationEnCoursExterne> optExter =
		// operationEncoursExterneService.findById(interId);
		/**
		 * Operation op = new Operation(); op. setOperationEnCoursId(interId);
		 * 
		 * // Utilisation d'ifPresentOrElse pour éviter le if/else classique
		 * optInter.ifPresentOrElse( inter -> {
		 * op.setImmatriculation(inter.getVehiculeTarer().getImmatriculation());
		 * op.setPeser1(inter.getVehiculeTarer().getPeser1());
		 * System.out.println("Object interne ------------- " + inter); }, () ->
		 * optExter.ifPresent(exter -> { if(exter.getPeser_vide()!= null) {
		 * op.setPeser2(exter.getPeser_charger()); System.out.println("Object exter en
		 * charge ------------- " + exter); } else {
		 * op.setPeser1(exter.getPeser_vide()); System.out.println("Object exter a vide
		 * ------------- " + exter); }
		 * 
		 * op.setImmatriculation(exter.getVehiculeExterne().getImmatriculation());
		 * 
		...
		return optInter.map(inter -> {
			...
			System.out.println("Object interne ------------- " + inter);
			return op;
		}).orElse(null); // Retourne null si optInter est vide

		/**
		 * ou bien sans le lambda
		 *  if (optInter.isPresent()) { OperationEnCoursInterne
		 * inter = optInter.get(); Operation op = new Operation();
		 * op.setOperationEnCoursId(interId);
		 * op.setImmatriculation(inter.getVehiculeTarer().getImmatriculation());
		 * op.setPeser1(inter.getVehiculeTarer().getPeser1());
		 * System.out.println("Object interne ------------- " + inter); return op; }
		 * else { return new Operation(); // ou return null; }
		 */
	}

	public Operation initFromInterneE(Integer interId) {
		/**
		 * if (interId == null) {
		 * 
		 * return new Operation(); }
		...
		// Récupération sous forme d'Optional
		// Optional<OperationEnCoursInterne> optInter =
		 ...
		 * 
		 **/
		return optExter.map(exter -> {
			Operation op = new Operation();
			op.setOperationEnCoursId(interId);
			op.setImmatriculation(exter.getVehiculeExterne().getImmatriculation());

			if (exter.getPeser_vide() != null) {
				op.setPeser1(exter.getPeser_vide());
				System.out.println("Object exter a vide ------------- " + exter);
			} else {
				op.setPeser2(exter.getPeser_charger());
				System.out.println("Object exter en charge ------------- " + exter);
			}
			return op;
		}).orElse(null);
	} // Fallback si aucun trouvé

	public void saveOperation(Operation operation, Long produitId, Long partenaireId, Integer typeOperationId,
			Authentication auth) {
		...
		Optional<OperationEnCoursInterne> optInterne = operationEnCoursInterneService
				.findById(operation.getOperationEnCoursId());
		Optional<OperationEnCoursExterne> optExterne = operationEncoursExterneService
				.findById(operation.getOperationEnCoursId());
		...
		if (optInterne.isPresent() && !optExterne.isPresent()) {
			...
			// par exemple, supprimer l'un ou lever une exception ou loguer un
			// avertissement.
			// Ici, on peut choisir de supprimer les deux (ou seulement l'un, selon la
			// logique métier)
			// operationEnCoursInterneService.deleteById(operation.getOperationEnCoursId());
			// operationEncoursExterneService.deleteById(operation.getOperationEnCoursId());
		} else {
			// Aucune entité trouvée, éventuellement loguer une erreur ou effectuer un
